...
// ---- SO-101 rig: six sliders driving the joint chain ----
// Pan and roll rotate around axes pointing at the viewer, so in this side view
// they are approximated by horizontal/vertical squash rather than a rotation.
const JOINTS = [
  { id: "pan",      label: "Base",     min: -60, max: 60,  start: 0,  unit: "°" },
  { id: "shoulder", label: "Shoulder", min: -35, max: 40,  start: 0,  unit: "°" },
  { id: "elbow",    label: "Elbow",    min: -50, max: 50,  start: 0,  unit: "°" },
  { id: "wrist",    label: "Wrist",    min: -55, max: 55,  start: 0,  unit: "°" },
  { id: "roll",     label: "Roll",     min: -90, max: 90,  start: 0,  unit: "°" },
  { id: "grip",     label: "Gripper",  min: 0,   max: 100, start: 25, unit: "%" },
];

const rig = document.querySelector(".robot-rig");

if (rig) {
  const controls = document.getElementById("rig-controls");
  const pose = Object.fromEntries(JOINTS.map(j => [j.id, j.start]));
  const parts = {
    pan: rig.querySelector(".ra-pan"),
    shoulder: rig.querySelector(".ra-shoulder"),
    elbow: rig.querySelector(".ra-elbow"),
    wrist: rig.querySelector(".ra-wrist"),
    jaws: rig.querySelector(".ra-jaws"),
    jawA: rig.querySelector(".ra-jaw-a"),
    jawB: rig.querySelector(".ra-jaw-b"),
  };

  function applyPose() {
    const rad = deg => (deg * Math.PI) / 180;
    // Turning the base away from the viewer narrows the arm rather than rotating it.
    parts.pan.style.transform = `scaleX(${Math.cos(rad(pose.pan)).toFixed(3)})`;
    parts.shoulder.style.transform = `rotate(${pose.shoulder}deg)`;
    parts.elbow.style.transform = `rotate(${pose.elbow}deg)`;
    parts.wrist.style.transform = `rotate(${pose.wrist}deg)`;
    parts.jaws.style.transform = `scaleY(${(0.35 + 0.65 * Math.abs(Math.cos(rad(pose.roll)))).toFixed(3)})`;
    const open = pose.grip / 100;
    parts.jawA.style.transform = `rotate(${(-16 * open).toFixed(1)}deg)`;
    parts.jawB.style.transform = `rotate(${(16 * open).toFixed(1)}deg)`;
  }
  ...
  JOINTS.forEach(joint => {
    ...
    label.htmlFor = `rig-${joint.id}`;

    const input = document.createElement("input");
    input.type = "range";
    input.id = `rig-${joint.id}`;
    input.min = joint.min;
    input.max = joint.max;
    input.value = joint.start;

    const out = document.createElement("output");
    out.textContent = joint.start + joint.unit;

    input.addEventListener("input", () => {
      if (!touched) { touched = true; rig.classList.remove("idle"); }
      pose[joint.id] = Number(input.value);
      out.textContent = input.value + joint.unit;
      applyPose();
    });
  ...
  document.getElementById("rig-reset").addEventListener("click", () => {
    JOINTS.forEach(joint => {
      pose[joint.id] = joint.start;
      const input = document.getElementById(`rig-${joint.id}`);
      input.value = joint.start;
      input.nextElementSibling.textContent = joint.start + joint.unit;
    });
    applyPose();
    touched = false;
